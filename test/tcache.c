...
void lRUBlockPut(int key, struct block* value) {
    ...
    if (cur != NULL) {
        ...
        TracePrintf( LOG, "[CACHE][LOG] Found the block, replace it with new value!\n");
        ...
        TracePrintf( LOG, "[CACHE][LOG] Full, evicting a blk\n");
        ...
        TracePrintf( LOG, "[CACHE][LOG] Didn't find the block, create a new entry in cache!\n");
...
struct block* lRUGetBlk(int key) {
    ...
    if (cur != NULL) { // found, move it to the head
        TracePrintf( LOG, "[CACHE][LOG] Found block, put it to the head...\n");
        HASH_DEL(blk_head, cur);
        HASH_ADD_INT(blk_head, key, cur);
        return cur->val;
    }
    // not found
    TracePrintf( LOG, "[CACHE][LOG] Can't find block!\n");
    return NULL;
}
...
main()
{
    TracePrintf( LOG, "Cache test in progress\n");
